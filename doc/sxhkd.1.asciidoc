...
If *sxhkd* receives a _SIGUSR2_ signal, it will toggle the grabbing state of all its bindings.

If *sxhkd* receives a _SIGALRM_ signal, it will abort its current chord chain.
